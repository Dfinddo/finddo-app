...
const ChatListAdmin = ((props: ChatListScreenProps): JSX.Element => {
	...
	const getChats = useCallback(async (): Promise<void> => {
		...
		try {
			const response = await finddoApi.get(`/chats/admin/all`, {
        params: {
          page: 1,
        },
			});
	...
	const handleExpandList = useCallback(async()=>{
      ...
      return;
		...
		try {
			const response = !index ? await finddoApi.get(`/chats/admin/all`, {
        params: {
			...
			ListEmptyComponent={
				...
				<View style={styles.emptyList}>
					...
					<Text style={styles.emptyListText}>
						Ainda não possui nenhuma conversa ativa. Tente recarregar a página.
					</Text>
				</View>}
				</>
			}
	...
	return (
		<Layout level="3">
				...
				<TabView
					...
					<Tab title='Serviços em execução'>
