...
export default class ValorServicoScreen extends Component {
	...
	formatarValorServico = valor => `R$ ${String((Math.round((valor + Number.EPSILON) * 100) / 100).toFixed(2))}`;
	...
	prepararPedidoWirecard = (pedido, uuid) => {
		...
		item.price = this.formatarValorServico(this.state.valorComTaxa).split("").slice(3)
			.join("")
			.replace(".", "")
			.replace(",", "");
		...
		const primaryReceiver = {
			...
			amount: {
				fixed: this.formatarValorServico(this.state.valorTaxa).split("").slice(3)
					.join("")
					.replace(".", "")
					.replace(",", ""),
			},
		...
		const secondaryReceiver = {
			...
			amount: {
				fixed: this.formatarValorServico(Number(this.state.valorServico)).split("").slice(3)
					.join("")
					.replace(".", "")
					.replace(",", ""),
			},
