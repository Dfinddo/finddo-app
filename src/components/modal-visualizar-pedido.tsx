	...
	render() {
		...
		if (this.state.order) {
			...
			if (this.state.order.start_order && this.state.order.urgencia === "semana") {
				...
				diaFim = `${dataFim.getDate()}/${Number(dataFim.getMonth()) + 1}/${dataFim.getFullYear()}`;
				...
				if (this.props.pedido.isShowingPedido) {
					...
					diaInicio = `${data.getDate()}/${Number(data.getMonth()) + 1}/${data.getFullYear()}`;
							...
							<View style={{alignItems: "center", justifyContent: "flex-start", paddingVertical: 13}}>
								...
								<Text style={this.visualizarPedidoStyle.titulos}>Fotos:</Text>
								{
									(() => {
										...
										if (this.props.pedido.isShowingPedido)
											fotos = this.props.pedido.pedidoCorrente.images.map((foto, index) => ({id: `${index}`, foto: {image: {uri: backendUrl + foto}}}));
										else {

											fotos = this.props.pedido.fotosPedido.map((foto, index) => ({id: `${index}`, foto}));
											...
											if (fotos.length === 0) {
												...
												fotos = fotosPedido.map((foto, index) => ({id: `${index}`, foto}));
										...
										if (fotosPedido.length > 0) {
											...
											return (
												<FlatList data={fotos} renderItem={({item}) => <Image source={item.foto} style={{width: 240, height: 240, marginTop: 10}} />} />
											);

										}

										return (
											<FlatList data={fotos} renderItem={({item}) => <Image source={item.foto.image} style={{width: 240, height: 240, marginTop: 10}} />} />
										);
